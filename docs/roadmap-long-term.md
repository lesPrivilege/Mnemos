...
7. ~~**字階與間距接線**~~ ✅ 字階完成（2026-07-25，feat/scale-wiring）：字階據實重導為九檔 rem 尺並全量接線（CSS 百處 + tailwind + JSX），死校增 `g1-fontsize` 探測器鎖單源，**Dynamic Type 就緒**。間距**已拒**大掃（記-18：無競爭源可統、無實賞可換、強接有損），`--sp-*` 據實重導為 2px 細網供新碼取用，既有手調值機會主義歸檔
