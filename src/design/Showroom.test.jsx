...
it('renders real panels and retains edited draft on failure, then resets memory without storage', async () => {
  ...
  fireEvent.submit(screen.getByRole('form', { name: '卡片草稿' }))
  expect(screen.getByRole('region', { name: '已保存卡片' }).textContent).toContain('保留这个问题')
...
it('returns to the second repeated paragraph and reports a missing source without losing quote', async () => {
  ...
  const matches = [...document.querySelectorAll('.showroom-article p')].filter(p => p.textContent === '记住结论并不等于能够解释结论。')
  ...
  fireEvent.click(screen.getByRole('button', { name: '保存卡片' }))
  fireEvent.click(screen.getByRole('button', { name: '查看原文' }))
  expect(screen.getByText('原文已不可用，摘句仍保留在卡片中。')).toBeTruthy()
